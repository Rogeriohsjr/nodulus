...
PKG-006 follow-up: actual `npm publish --dry-run --access public --json --ignore-scripts` exited zero but warned about normalizing the CLI bin mapping. The regression failed on that warning before changing `./dist/bin.js` to canonical `dist/bin.js`. The corrected real dry run and all seven package tests passed, followed by lint/typecheck and 115 runtime scenarios in `npm run check`. Sol accepted RED and GREEN. This remains nonpublishing evidence; npm's warning did not actually remove the bin entry.
