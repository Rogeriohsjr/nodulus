...
The Cursor adapter probes `--version` and `status --format json`, requires `isAuthenticated: true`, then runs print mode with JSON output, `--trust`, and the project workspace. It sends the complete prompt through stdin, avoiding Windows argument-length limits and avoiding a model tool call merely to load the prompt. `--trust` suppresses the non-interactive workspace prompt; Nodulus does not pass `--force` or `--yolo`. It accepts only the documented success envelope fields `type: "result"`, `subtype: "success"`, `is_error: false`, and string `result`. Windows live verification used Cursor Agent `2026.09.23-86fc751`; no general minimum version has been established, so an unparseable version is rejected while other parseable versions remain allowed.
