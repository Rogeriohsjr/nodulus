...
**Status:** complete locally on Windows; independently accepted by Sol. **Prerequisite:** 01-initialize. **Method:** TDD.
...
- [x] REQ-001 acceptance verified and evidence recorded.
...
- [x] REQ-002 acceptance verified and evidence recorded.
...
- [x] REQ-003 acceptance verified and evidence recorded.

### REQ-004: Reject broken definitions

Given missing instructions, invalid workflow schema, unknown profile, or invalid mapping, when preflight runs, then it returns an actionable `CONFIGURATION_INVALID` diagnostic with `runId: null` before creating a durable run, and no provider is invoked.

- [x] REQ-004 acceptance verified and evidence recorded.
...
- [x] REQ-005 acceptance verified and evidence recorded.
...
- [x] REQ scenarios cover real files and exact preserved request text.
- [x] Paths and hashes are asserted from disk; no filesystem mocks.
- [x] Keep incomplete run command behavior explicit until slice 03.
