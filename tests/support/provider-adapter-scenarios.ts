...
function providerFixtureSource(kind: FixtureProviderKind, logPath: string, probePath: string, invocationPath: string, controlPath: string): string {
  return [
    ...
    "if (kind === 'cursor' && args[0] === 'status' && args.includes('--format') && readFlag('--format') === 'json') { logProbe(); if (mode === 'auth-failure') { process.stderr.write('fixture status check failed'); process.exit(1); } if (mode === 'auth-json-unauthenticated') { process.stdout.write(JSON.stringify({ status: 'unauthenticated', isAuthenticated: false, hasAccessToken: false, hasRefreshToken: false, message: 'Not logged in' }, null, 2)); process.exit(0); } process.stdout.write(JSON.stringify({ status: 'authenticated', isAuthenticated: true, hasAccessToken: true, hasRefreshToken: true })); process.exit(0); }",
    "let promptFile;",
